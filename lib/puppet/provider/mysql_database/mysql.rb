...
Puppet::Type.type(:mysql_database).provide(:mysql, parent: Puppet::Provider::Mysql) do
  ...
  def create
    self.class.mysql_caller("set session sql_log_bin=0; create database if not exists `#{@resource[:name]}` character set `#{@resource[:charset]}` collate `#{@resource[:collate]}`", 'regular')
  ...
  def destroy
    self.class.mysql_caller("set session sql_log_bin=0; drop database if exists `#{@resource[:name]}`", 'regular')
  ...
  def charset=(value)
    self.class.mysql_caller("set session sql_log_bin=0; alter database `#{resource[:name]}` CHARACTER SET #{value}", 'regular')
    @property_hash[:charset] = value
    (charset == value) ? (return true) : (return false)
  end

  def collate=(value)
    self.class.mysql_caller("set session sql_log_bin=0; alter database `#{resource[:name]}` COLLATE #{value}", 'regular')
